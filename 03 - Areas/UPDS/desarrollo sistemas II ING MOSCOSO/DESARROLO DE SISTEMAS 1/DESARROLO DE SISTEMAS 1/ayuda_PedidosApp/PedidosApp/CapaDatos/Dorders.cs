...
namespace CapaDatos
{
    public class Dorders : DbConnection
    {
        ...
        public string ActualizarEstadoPedido(int orderId, int nuevoEstado, int usuarioId)
        {
            string rpta = string.Empty;
            using (SqlConnection connection = GetConnection())
            {
                ...
                using (SqlCommand command = new SqlCommand())
                {
                    try
                    {
                        ...
                        command.Parameters.AddWithValue("@new_status", nuevoEstado);
        ...
        public DataTable EstadisticasPedidosEstado(DateTime? fechaInicio = null, DateTime? fechaFin = null)
        {
            DataTable DtResultado = new DataTable("estadisticas_estados");
            using (SqlConnection connection = GetConnection())
            {
                connection.Open();
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    {
                        command.Connection = connection;
                        command.CommandText = "sp_estadisticas_pedidos_estado";
                        command.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable PedidosRequierenAtencion(int diasLimite = 3)
        {
            DataTable DtResultado = new DataTable("pedidos_atencion");
            using (SqlConnection connection = GetConnection())
            {
                connection.Open();
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    {
                        command.Connection = connection;
                        command.CommandText = "sp_pedidos_requieren_atencion";
                        command.CommandType = CommandType.StoredProcedure;

                        command.Parameters.AddWithValue("@dias_limite", diasLimite);
        ...
        public DataTable DashboardEstadosPedidos()
        {
            DataTable DtResultado = new DataTable("dashboard_estados");
            using (SqlConnection connection = GetConnection())
            {
                connection.Open();
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    {
                        command.Connection = connection;
                        command.CommandText = "SELECT * FROM vw_dashboard_estados_pedidos";
