...
namespace PedidosApp
{
    public partial class FrmDashboard : Form
    {
        ...
        {
            InitializeComponent();
            connection = new SqlConnection(DbConnection.cn);
        ...
        private void CargarEstadisticas()
        {
            try
            {
                ...
                // Ventas totales (considerando descuentos)
                query = "SELECT ISNULL(SUM(quantity * price * (1 - discount)), 0) FROM order_items";
                cmd = new SqlCommand(query, connection);
                object result = cmd.ExecuteScalar();
                decimal ventasTotales = result != DBNull.Value ? Convert.ToDecimal(result) : 0;
                lblVentasTotales.Text = ventasTotales.ToString("C");

                // Productos disponibles (en lugar de stock bajo)
        ...
        private void CargarUltimosPedidos()
        {
            try
            {
                ...
                string query = @"SELECT TOP 10 o.order_id, c.first_name + ' ' + c.last_name as Cliente, 
                                 o.order_date, ISNULL(SUM(oi.quantity * oi.price * (1 - oi.discount)), 0) as Total
                                 FROM orders o
