    ...
    public partial class FrmUsers : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (EsNuevo)
                    {
                        rpta = Nusers.Insertar(txtNombre.Text.Trim(), txtClave.Text.Trim(), txtEmail.Text.Trim());
                    }
                    else
                    {
        ...
        private void dataListado_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                txtNombre.Text = Convert.ToString(dataListado.CurrentRow.Cells["usuario_name"].Value);
                txtClave.Text = Convert.ToString(dataListado.CurrentRow.Cells["usuario_clave"].Value);
                txtEmail.Text = Convert.ToString(dataListado.CurrentRow.Cells["usuario_email"].Value);
                tabControl1.SelectedIndex = 1;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al obtener los datos del usuario: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
