-- =============================================
-- SCRIPT DE MIGRACIÓN PARA phpMyAdmin
-- Copiar y pegar este código en la pestaña SQL de phpMyAdmin
-- =============================================

-- Crear la base de datos (ejecutar solo si no existe)
CREATE DATABASE IF NOT EXISTS bike_store CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
...
-- Eliminar tablas si existen (orden importante por las claves foráneas)
...
-- Insertar productos de ejemplo (bicicletas)
...
-- Insertar items de pedidos de ejemplo
INSERT INTO order_items (order_id, product_id, quantity, price, discount) VALUES
-- Pedido 1: Carlos Rodríguez
(1, 1, 1, 2500.00, 0.0000), -- Trek X1
(1, 3, 2, 800.00, 0.1000),  -- BMX con 10% descuento
-- Pedido 2: María González
(2, 2, 1, 1200.00, 0.0000), -- City Comfort
-- Pedido 3: José Mamani
(3, 4, 1, 4500.00, 0.0500), -- E-Bike con 5% descuento
(3, 6, 1, 1800.00, 0.0000), -- Urban Explorer
-- Pedido 4: Ana Quispe
(4, 5, 1, 3200.00, 0.0000), -- Speed Racing
-- Pedido 5: Carlos Rodríguez (segundo pedido)
(5, 2, 2, 1200.00, 0.0800); -- 2 City Comfort con 8% descuento
...
-- Vista para pedidos con información completa del cliente
...
-- Vista para productos con estadísticas de ventas
...
UNION ALL
SELECT 'Productos' as TABLA, COUNT(*) as REGISTROS FROM products
UNION ALL
SELECT 'Pedidos' as TABLA, COUNT(*) as REGISTROS FROM orders
UNION ALL
SELECT 'Items de Pedidos' as TABLA, COUNT(*) as REGISTROS FROM order_items;
...
SELECT 
    ...
    email as EMAIL,
    'Cambiar contraseña después del primer login' as NOTA
FROM usuarios 
WHERE usuario = 'admin';