...
// segunda sesion del welcome-section
// Function to save services to local storage
function saveServiceToLocalStorage(serviceData) {
    let services = JSON.parse(localStorage.getItem('registeredServices') || '[]');
 ...
 function loadServicesFromLocalStorage() {
    ...
    services.forEach(service => {
        ...
        newRow.innerHTML = `
            <td>${service.Especialidades}</td>
            <td>${service.Especialista}</td>
            <td>${service.Precio}</td>
            <td>${service.Duracion_Estimada}</td>
            <td>${service.Categoria}</td>
            <td>${service.Turno}</td>
            <td>${service.Cod_Historial}</td>
        `;
    });
 }
 ...
 document.querySelector('.proforma-form').addEventListener('submit', function(event) {
    ...
    const serviceData = {
        ...
        Cod_Historial: document.getElementById('Cod_Historial').value
    };
 
    // Save to local storage for immediate display
    saveServiceToLocalStorage(serviceData);
 
    // Add row to the table immediately
    const tableBody = document.getElementById('serviceTableBody');
    const newRow = tableBody.insertRow();
    newRow.innerHTML = `
        <td>${serviceData.Especialidades}</td>
        <td>${serviceData.Especialista}</td>
        <td>${serviceData.Precio}</td>
        <td>${serviceData.Duracion_Estimada}</td>
        <td>${serviceData.Categoria}</td>
        <td>${serviceData.Turno}</td>
        <td>${serviceData.Cod_Historial}</td>
    `;
 
    // Submit the form to PHP for database insertion
    this.submit();
 });
 
 // Load existing services from local storage when page loads
 document.addEventListener('DOMContentLoaded', function() {
